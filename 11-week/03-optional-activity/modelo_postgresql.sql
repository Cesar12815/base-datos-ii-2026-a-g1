...
CREATE ROLE app_admin WITH LOGIN ENCRYPTED PASSWORD 'admin_password';
ALTER ROLE app_admin CREATEDB CREATEROLE;

CREATE ROLE app_user WITH LOGIN ENCRYPTED PASSWORD 'user_password';
CREATE ROLE app_readonly WITH NOLOGIN;
CREATE ROLE app_operator WITH LOGIN ENCRYPTED PASSWORD 'operator_password';

-- Grant schema permissions
GRANT USAGE ON SCHEMA public TO app_user, app_operator, app_readonly;
...
GRANT SELECT, INSERT, UPDATE ON reservation, sale, ticket, payment TO app_operator;
GRANT SELECT ON flight, aircraft, airline TO app_operator;

-- Grant sequence permissions
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO app_user, app_operator;

-- ============================================
-- SYSTEM FUNCTIONS
-- ============================================

-- System function: Generate unique account numbers
...
BEGIN
    ...
    random_part := lpad(floor(random() * 10000000000)::text, 10, '0');
    account_number := prefix || timestamp_part || random_part;
    RETURN account_number;
END;
$$ LANGUAGE plpgsql IMMUTABLE;

-- System function: Calculate age from birth date
CREATE OR REPLACE FUNCTION calculate_age(birth_date date)
RETURNS integer AS $$
BEGIN
    RETURN EXTRACT(YEAR FROM age(now(), birth_date))::integer;
END;
$$ LANGUAGE plpgsql IMMUTABLE;

-- System function: Get flight duration in hours
CREATE OR REPLACE FUNCTION get_flight_duration_hours(
    departure_time timestamptz,
    arrival_time timestamptz
)
RETURNS numeric AS $$
BEGIN
    RETURN EXTRACT(EPOCH FROM (arrival_time - departure_time)) / 3600;
END;
$$ LANGUAGE plpgsql IMMUTABLE;

-- System function: Validate email format
CREATE OR REPLACE FUNCTION is_valid_email(email_text varchar)
RETURNS boolean AS $$
BEGIN
...
-- User function: Get customer loyalty tier
CREATE OR REPLACE FUNCTION get_customer_tier(customer_id uuid)
RETURNS TABLE(
    tier_name varchar,
...
BEGIN
    ...
    SELECT 
        lt.tier_name,
        lt.priority_level,
        COALESCE(SUM(CASE WHEN mt.transaction_type = 'EARN' THEN mt.miles_delta
                         WHEN mt.transaction_type = 'REDEEM' THEN -mt.miles_delta
                         ELSE 0 END), 0)::integer
    FROM customer c
    JOIN loyalty_account la ON c.customer_id = la.customer_id
    ...
    WHERE c.customer_id = get_customer_tier.customer_id
        AND lat.expires_at IS NULL
    GROUP BY lt.tier_name, lt.priority_level;
END;
$$ LANGUAGE plpgsql STABLE;

-- User function: Calculate ticket price with taxes
CREATE OR REPLACE FUNCTION calculate_ticket_total(
    ticket_id uuid,
...
BEGIN
    ...
    SELECT 
        f.base_amount,
        ROUND(f.base_amount * (tax_rate_percent / 100), 2),
        ROUND(f.base_amount * (1 + tax_rate_percent / 100), 2)
    FROM ticket t
    JOIN fare f ON t.fare_id = f.fare_id
    WHERE t.ticket_id = calculate_ticket_total.ticket_id;
END;
$$ LANGUAGE plpgsql STABLE;

-- User function: Get available seats for flight segment
CREATE OR REPLACE FUNCTION get_available_seats(flight_segment_id uuid)
RETURNS TABLE(
...
BEGIN
    RETURN QUERY
    SELECT 
        aseat.aircraft_seat_id,
        aseat.seat_row_number::text || aseat.seat_column_code,
        aseat.is_window,
        aseat.is_aisle
    FROM aircraft_seat aseat
    WHERE aseat.aircraft_cabin_id IN (
        SELECT acab.aircraft_cabin_id
        ...
        WHERE fs.flight_segment_id = get_available_seats.flight_segment_id
    )
    AND NOT EXISTS (
        SELECT 1
        FROM seat_assignment sa
        WHERE sa.aircraft_seat_id = aseat.aircraft_seat_id
        AND sa.flight_segment_id = get_available_seats.flight_segment_id
    );
END;
$$ LANGUAGE plpgsql STABLE;
...
-- Trigger: Update updated_at timestamp on person
...
-- Trigger: Update updated_at timestamp on user_account
...
CREATE TRIGGER trg_update_user_account_timestamp
BEFORE UPDATE ON user_account
FOR EACH ROW
EXECUTE FUNCTION update_user_account_timestamp();

-- Trigger: Update flight status based on actual times
CREATE OR REPLACE FUNCTION update_flight_status_on_segment_change()
RETURNS TRIGGER AS $$
DECLARE
    all_segments_completed boolean;
    flight_status_id uuid;
BEGIN
    -- Check if all segments have actual departure and arrival times
    SELECT COUNT(*) = COUNT(CASE WHEN actual_departure_at IS NOT NULL 
                                      AND actual_arrival_at IS NOT NULL THEN 1 END)
    INTO all_segments_completed
    FROM flight_segment
    WHERE flight_id = NEW.flight_id;
    
    IF all_segments_completed THEN
        -- Get the "COMPLETED" status
        SELECT fs.flight_status_id INTO flight_status_id
        FROM flight_status fs
        WHERE fs.status_code = 'COMPLETED'
        LIMIT 1;
        
        UPDATE flight SET status = flight_status_id WHERE flight_id = NEW.flight_id;
    END IF;
...
CREATE TRIGGER trg_update_flight_status
AFTER UPDATE ON flight_segment
FOR EACH ROW
WHEN (OLD.actual_departure_at IS DISTINCT FROM NEW.actual_departure_at
      OR OLD.actual_arrival_at IS DISTINCT FROM NEW.actual_arrival_at)
EXECUTE FUNCTION update_flight_status_on_segment_change();

-- Trigger: Log ticket issuance for audit
CREATE TABLE audit_ticket_issued (
...
CREATE OR REPLACE FUNCTION audit_ticket_issuance()
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO audit_ticket_issued (ticket_id, ticket_number, action)
    VALUES (NEW.ticket_id, NEW.ticket_number, 'ISSUED');
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER trg_audit_ticket_issuance
AFTER INSERT ON ticket
FOR EACH ROW
EXECUTE FUNCTION audit_ticket_issuance();
...
-- Procedure: Create new customer with person
CREATE OR REPLACE FUNCTION sp_create_customer(
    p_airline_id uuid,
    p_first_name varchar,
    p_last_name varchar,
    p_person_type_id uuid,
    p_customer_category_id uuid DEFAULT NULL
)
RETURNS TABLE(customer_id uuid, person_id uuid, message varchar) AS $$
DECLARE
    v_person_id uuid;
    v_customer_id uuid;
BEGIN
    ...
    VALUES (p_person_type_id, p_first_name, p_last_name)
    RETURNING person.person_id INTO v_person_id;
    
    -- Create customer
    INSERT INTO customer (airline_id, person_id, customer_category_id)
    VALUES (p_airline_id, v_person_id, p_customer_category_id)
    RETURNING customer.customer_id INTO v_customer_id;
    
    RETURN QUERY SELECT v_customer_id, v_person_id, 'Customer created successfully'::varchar;
END;
$$ LANGUAGE plpgsql;
...
RETURNS TABLE(refund_id uuid, status varchar, message varchar) AS $$
DECLARE
    v_refund_id uuid;
...
BEGIN
    ...
    IF v_payment_amount IS NULL THEN
        RETURN QUERY SELECT NULL::uuid, 'ERROR'::varchar, 'Payment not found'::varchar;
        RETURN;
    END IF;
    
    IF p_amount > v_payment_amount THEN
        RETURN QUERY SELECT NULL::uuid, 'ERROR'::varchar, 'Refund amount exceeds payment amount'::varchar;
        RETURN;
    END IF;
    
    -- Create refund record
    INSERT INTO refund (payment_id, refund_reference, amount, refund_reason, requested_at)
    VALUES (
        p_payment_id,
        'REF-' || to_char(now(), 'YYYYMMDDHH24MISSUS'),
        p_amount,
        p_refund_reason,
        now()
    )
    RETURNING refund.refund_id INTO v_refund_id;
    
    -- Update payment status
    UPDATE payment
    SET payment_status_id = (SELECT payment_status_id FROM payment_status WHERE status_code = 'REFUNDED')
    WHERE payment_id = p_payment_id;
    
    RETURN QUERY SELECT v_refund_id, 'SUCCESS'::varchar, 'Refund processed successfully'::varchar;
END;
$$ LANGUAGE plpgsql;

-- Procedure: Assign seat to passenger
CREATE OR REPLACE FUNCTION sp_assign_seat(
    p_ticket_segment_id uuid,
    p_flight_segment_id uuid,
    p_aircraft_seat_id uuid,
    p_assignment_source varchar DEFAULT 'MANUAL'
)
RETURNS TABLE(seat_assignment_id uuid, status varchar, message varchar) AS $$
DECLARE
...
BEGIN
    ...
    IF EXISTS (
        ...
        AND aircraft_seat_id = p_aircraft_seat_id
    ) THEN
        RETURN QUERY SELECT NULL::uuid, 'ERROR'::varchar, 'Seat already assigned'::varchar;
        RETURN;
    END IF;
    
    -- Get seat number
    SELECT seat_row_number::text || seat_column_code
    INTO v_seat_number
    FROM aircraft_seat
    ...
    RETURNING seat_assignment.seat_assignment_id INTO v_seat_assignment_id;
    
    RETURN QUERY SELECT v_seat_assignment_id, 'SUCCESS'::varchar, 
        'Seat ' || v_seat_number || ' assigned successfully'::varchar;
END;
$$ LANGUAGE plpgsql;

-- Procedure: Update flight segment actual times
CREATE OR REPLACE FUNCTION sp_update_flight_segment_times(
    p_flight_segment_id uuid,
    p_actual_departure_at timestamptz DEFAULT NULL,
    p_actual_arrival_at timestamptz DEFAULT NULL
)
RETURNS TABLE(flight_segment_id uuid, status varchar, message varchar) AS $$
DECLARE
    v_scheduled_departure timestamptz;
    v_scheduled_arrival timestamptz;
BEGIN
    -- Get scheduled times
    SELECT scheduled_departure_at, scheduled_arrival_at
    INTO v_scheduled_departure, v_scheduled_arrival
    FROM flight_segment
    WHERE flight_segment_id = p_flight_segment_id;
    
    -- Validate times
    IF p_actual_departure_at IS NOT NULL AND p_actual_departure_at < v_scheduled_departure THEN
        RETURN QUERY SELECT p_flight_segment_id, 'ERROR'::varchar, 
            'Actual departure cannot be before scheduled departure'::varchar;
        RETURN;
    END IF;
    ...
    SET 
        actual_departure_at = COALESCE(p_actual_departure_at, actual_departure_at),
        actual_arrival_at = COALESCE(p_actual_arrival_at, actual_arrival_at),
        updated_at = now()
    WHERE flight_segment_id = p_flight_segment_id;
    
    RETURN QUERY SELECT p_flight_segment_id, 'SUCCESS'::varchar, 'Flight segment times updated'::varchar;
END;
$$ LANGUAGE plpgsql;

-- Procedure: Get customer reservation history
CREATE OR REPLACE FUNCTION sp_get_customer_reservations(p_customer_id uuid)
RETURNS TABLE(
    reservation_id uuid,
